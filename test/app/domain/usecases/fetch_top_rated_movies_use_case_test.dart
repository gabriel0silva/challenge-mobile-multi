...
void main() {
  ...
  late FetchTopRatedMoviesUseCase mockFetchTopRatedMoviesUseCase;
  ...
  setUp(() {
    ...
    mockFetchTopRatedMoviesUseCase = FetchTopRatedMoviesUseCase(mockRepository);
  });

  group('FetchTopRatedMoviesUseCase', () {
    ...
    test('call returns MoviesModel when repository returns data', () async {
      ...
      when(() => mockRepository.fetchTopRatedMovies()).thenAnswer((_) async => moviesModel);

      final result = await mockFetchTopRatedMoviesUseCase.call();

      expect(result, isNotNull);
      expect(result, isA<MoviesModel>());
      verify(() => mockRepository.fetchTopRatedMovies()).called(1);
    });

    test('call returns null when repository returns null', () async {
      when(() => mockRepository.fetchTopRatedMovies()).thenAnswer((_) async => null);
